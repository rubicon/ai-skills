...
| Disconnect unused MCP servers | Run `/mcp` at the start of each session and disconnect anything you won't use. One connected server can load ~17,600 tokens/message whether you touch it or not. Prefer a CLI over an MCP server when both exist. |
...
| Leaving MCP servers connected "just in case" | Every connected server loads its full tool definitions into context on every message, whether it's used or not |
